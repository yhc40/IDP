...
void Rotate_left_until_match(){
 ...
 void color_path(){
    if (color_hold==1){
        Rotate_left_inplace(190,2110);
        Brake(100);
    };
    if (color_hold==0){
        Rotate_right_inplace(190,1850);
        Brake(100);
    };
}
void loop() 
...
//      Brake(500);
  if (distance <=45 && load_carrying == 0){
      sweep();
      open_arm(30,3,120);
      delay(800);
//      color_detection();
